        ...
        if isinstance(value, dict):
            ...
            for i, v in enumerate(value):
                if isinstance(v, dict):
                    items.update(flatten_dict(v, separator, f"{new_key}{separator}{i}"))
                else:
                    items[f"{new_key}{separator}{i}"] = v
        else:
            items[new_key] = value
...
@cli.command()
@click.option('--address', '-a', default='127.0.0.1', required=True,
...
def configure(address: str, username: str, password: str):
    ...
    click.secho("⚠️ Warning: Password stored in plain text!", fg='red')
